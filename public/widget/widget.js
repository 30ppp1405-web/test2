 ...
 * Usage: <script src="/widget/widget.js"></script>
 */
(function() {
  'use strict';
  ...
  let state = {
    open: false, phase: 'pre',
    visitorId: localStorage.getItem('lc_visitor_id') || null,
    conversationId: null, messages: [], agentsOnline: false,
  ...
  const CSS = `
    ...
    #lc-widget { position:fixed; z-index:2147483647; bottom:24px; right:24px; display:flex; flex-direction:column; align-items:flex-end; gap:12px; }
    #lc-bubble { width:60px; height:60px; border-radius:50%; background:${config.primaryColor}; cursor:pointer; display:flex; align-items:center; justify-content:center; box-shadow:0 4px 20px rgba(99,102,241,.5); transition:transform .2s,box-shadow .2s; position:relative; border:none; outline:none; }
    ...
    #lc-badge { position:absolute; top:-4px; right:-4px; background:#ef4444; color:#fff; font-size:11px; font-weight:700; width:20px; height:20px; border-radius:50%; display:none; align-items:center; justify-content:center; border:2px solid #fff; }
    #lc-window { width:380px; height:580px; border-radius:20px; background:#fff; box-shadow:0 20px 60px rgba(0,0,0,.2); display:flex; flex-direction:column; overflow:hidden; transform-origin:bottom right; transition:transform .3s cubic-bezier(.34,1.56,.64,1),opacity .3s; transform:scale(0.8) translateY(20px); opacity:0; pointer-events:none; }
    #lc-window.open { transform:scale(1) translateY(0); opacity:1; pointer-events:all; }
    #lc-header { background:${config.primaryColor}; padding:20px; color:#fff; flex-shrink:0; }
    ...
    .lc-av { width:36px; height:36px; border-radius:50%; border:2px solid rgba(255,255,255,.4); background:rgba(255,255,255,.2); display:flex; align-items:center; justify-content:center; font-size:14px; font-weight:700; color:#fff; margin-left:-8px; }
    ...
    .lc-close-btn { background:rgba(255,255,255,.15); border:none; border-radius:50%; width:32px; height:32px; cursor:pointer; color:#fff; font-size:18px; display:flex; align-items:center; justify-content:center; flex-shrink:0; transition:background .2s; }
    ...
    .lc-field input { width:100%; padding:10px 14px; border:1.5px solid #e2e8f0; border-radius:10px; font-size:14px; color:#1e293b; outline:none; transition:border-color .2s; background:#f8fafc; }
    .lc-field input:focus { border-color:${config.primaryColor}; background:#fff; }
    .lc-start-btn { width:100%; padding:13px; background:${config.primaryColor}; color:#fff; border:none; border-radius:10px; font-size:15px; font-weight:600; cursor:pointer; transition:opacity .2s; }
    ...
    #lc-messages { flex:1; overflow-y:auto; padding:16px; display:flex; flex-direction:column; gap:10px; background:#f8fafc; }
    ...
    .lc-msg-av { width:28px; height:28px; border-radius:50%; flex-shrink:0; background:${config.primaryColor}; display:flex; align-items:center; justify-content:center; font-size:11px; font-weight:700; color:#fff; }
    ...
    .lc-bubble { padding:9px 13px; border-radius:16px; font-size:14px; line-height:1.5; word-break:break-word; }
    ...
    .lc-file-link { display:flex; align-items:center; gap:6px; text-decoration:none; font-size:13px; padding:9px 13px; border-radius:16px; }
    ...
    #lc-text-input { flex:1; padding:9px 13px; border:1.5px solid #e2e8f0; border-radius:10px; font-size:14px; resize:none; outline:none; font-family:inherit; max-height:100px; line-height:1.5; color:#1e293b; background:#f8fafc; transition:border-color .2s; }
    #lc-text-input:focus { border-color:${config.primaryColor}; background:#fff; }
    #lc-text-input::placeholder { color:#94a3b8; }
    #lc-send-btn { width:36px; height:36px; background:${config.primaryColor}; border:none; border-radius:10px; color:#fff; cursor:pointer; font-size:16px; display:flex; align-items:center; justify-content:center; flex-shrink:0; transition:opacity .2s; }
    ...
    #lc-pulse { position:absolute; width:100%; height:100%; border-radius:50%; background:${config.primaryColor}; animation:lc-pulse 2s infinite; pointer-events:none; display:none; }
    #lc-bubble.has-unread #lc-pulse { display:block; }
    @media (max-width:420px) { #lc-window { width:calc(100vw - 16px); height:calc(100vh - 90px); } }
  `;
  ...
  function buildWidget() {
    ...
    container.innerHTML = `
      <div id="lc-window">
        <div id="lc-header">
          <div class="lc-header-top">
            <div class="lc-team">
              <div class="lc-avatars">${config.teamAvatars.map(a => `<div class="lc-av">${a}</div>`).join('')}</div>
              <div class="lc-team-info">
                <div class="name">${config.teamName}</div>
                <div class="status" id="lc-status"><span class="lc-status-dot" id="lc-status-dot"></span><span id="lc-status-text">Connecting…</span></div>
              </div>
            </div>
        ...
        <div id="lc-prechat">
          <div class="lc-prechat-inner">
            ...
            <div class="lc-field"><label>Your name <span style="color:#ef4444">*</span></label><input type="text" id="lc-name-input" placeholder="John Doe" autocomplete="name" /></div>
            <div class="lc-field"><label>Email address</label><input type="email" id="lc-email-input" placeholder="john@example.com" autocomplete="email" /></div>
        ...
        <div id="lc-chat" style="display:none;flex:1;flex-direction:column;">
          ...
          <div id="lc-resolved-banner">✅ This conversation has been resolved.</div>
          <div id="lc-input-area">
            ...
            <div class="lc-powered">Powered by LiveChat</div>
          </div>
      ...
      <button id="lc-bubble" aria-label="Open chat">
        <div id="lc-pulse"></div>
        <svg class="icon-chat" width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
        <svg class="icon-close" width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M18 6 6 18M6 6l12 12" stroke="white" stroke-width="2.5" stroke-linecap="round"/></svg>
        <div id="lc-badge"></div>
      </button>`;
    ...
    document.getElementById('lc-text-input').addEventListener('keydown', e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } });
    ...
    document.getElementById('lc-file-input').addEventListener('change', async (e) => {
      ...
      await uploadAndSend(file); e.target.value = '';
    });
  ...
  function initSocket() {
    ...
    socket.on('connect', () => { state.connected = true; if (state.visitorId) resumeVisitor(); });
    socket.on('disconnect', () => { state.connected = false; updateStatus(false); });
    socket.on('visitor:ready', ({ visitorId, conversationId, messages, agentsOnline }) => {
      state.visitorId = visitorId; state.conversationId = conversationId;
      state.messages = messages; state.agentsOnline = agentsOnline;
      localStorage.setItem('lc_visitor_id', visitorId);
      updateStatus(agentsOnline); renderMessages(messages); showChatView();
    });
    socket.on('message:new', (msg) => {
      state.messages.push(msg); appendMessage(msg);
      if (msg.from === 'agent' && !state.open) { state.unread++; updateBadge(); }
      hideTyping();
    });
    socket.on('agent:typing', ({ typing }) => {
      if (typing) showTyping(); else hideTyping();
      clearTimeout(typingTimer);
  ...
  function resumeVisitor() {
    socket.emit('visitor:init', { visitorId: state.visitorId, name: state.name, email: state.email, page: window.location.href });
  }

  function toggleWidget() { state.open ? closeWidget() : openWidget(); }
  function openWidget() {
    state.open = true; state.unread = 0; updateBadge();
    ...
    if (state.phase === 'chat') setTimeout(() => { const m = document.getElementById('lc-messages'); m.scrollTop = m.scrollHeight; }, 50);
  }
  ...
  function startChat() {
    ...
    state.name = name; state.email = document.getElementById('lc-email-input').value.trim();
    ...
    if (!socket) { loadSocketIO(() => { initSocket(); socket.on('connect', resumeVisitor); }); }
    else resumeVisitor();
  }
  ...
  function renderMessages(messages) {
    const area = document.getElementById('lc-messages'); area.innerHTML = '';
    let lastDate = '';
    messages.forEach(msg => {
      const d = new Date(msg.timestamp).toLocaleDateString();
      if (d !== lastDate) { lastDate = d; const sep = document.createElement('div'); sep.className = 'lc-date-sep'; sep.innerHTML = `<span>${d === new Date().toLocaleDateString() ? 'Today' : d}</span>`; area.appendChild(sep); }
      area.appendChild(buildMsgEl(msg));
    });
    area.scrollTop = area.scrollHeight;
  }

  function appendMessage(msg) { const area = document.getElementById('lc-messages'); area.appendChild(buildMsgEl(msg)); area.scrollTop = area.scrollHeight; }

  function buildMsgEl(msg) {
    const isVisitor = msg.from === 'visitor';
    const row = document.createElement('div'); row.className = `lc-msg-row ${msg.from}`;
    ...
    row.innerHTML = `<div class="lc-msg-av" style="background:${avatarColor}">${avatarText}</div><div class="lc-bubble-wrap"><span class="lc-msg-name">${isVisitor ? 'You' : esc(msg.agentName||'Support')}</span>${bodyHtml}<span class="lc-msg-time">${fmtTime(msg.timestamp)}</span></div>`;
    return row;
  }

  function sendMessage() {
    if (!socket || !state.connected || !state.conversationId) return;
    const inp = document.getElementById('lc-text-input'); const text = inp.value.trim(); if (!text) return;
    socket.emit('visitor:send', { text });
    appendMessage({ id: Date.now(), from: 'visitor', text, timestamp: new Date().toISOString() });
  ...
  function showTyping() {
    ...
    if (!el) { el = document.createElement('div'); el.id = 'lc-typing-row'; el.className = 'lc-typing-row'; el.innerHTML = `<div class="lc-msg-av" style="background:#6366f1">S</div><div class="lc-typing-dots"><span></span><span></span><span></span></div>`; document.getElementById('lc-messages').appendChild(el); }
    document.getElementById('lc-messages').scrollTop = 9999;
  }
  function hideTyping() { const el = document.getElementById('lc-typing-row'); if (el) el.remove(); }

  function updateStatus(online) {
    const dot = document.getElementById('lc-status-dot'); const txt = document.getElementById('lc-status-text');
    if (!dot || !txt) return;
  ...
  function updateBadge() {
    const badge = document.getElementById('lc-badge'); const bubble = document.getElementById('lc-bubble');
    if (state.unread > 0) { badge.textContent = state.unread > 9 ? '9+' : state.unread; badge.style.display = 'flex'; bubble.classList.add('has-unread'); }
    else { badge.style.display = 'none'; bubble.classList.remove('has-unread'); }
  }
  ...
  function build() {
    buildWidget();
    if (state.visitorId && state.name) { loadSocketIO(() => { initSocket(); socket.on('connect', resumeVisitor); }); }
  }
